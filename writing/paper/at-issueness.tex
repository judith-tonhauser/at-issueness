...
\section{Introduction \label{sec:1_introduction}}
  ...
  % On the diagnostics side (Question i), empirical differences between the diagonistics have been discussed in \citealt{tonhauser_diagnosing_2012,snider_at-issuenessne_2017,snider_anaphoric_2017,koev_notions_2018,tonhauser_how_2018,korotkova_evidential_2020}.
  ...
  \begin{enumerate}
    ...
    \item  A final appositive cannot address the QUD,\footnote{
      ...
      \citealt{koev_notions_2018} suggests that English medial appositive NRRCs, though not-at-issue under the direct-dissent test, can behave as at-issue under the QUD diagnostic. This is in line with \citepos{tonhauser_diagnosing_2012} findings for Paraguayan Guaraní medial appositive DPs: These are not-at-issue on most diagnostics tested there (including direct dissent, and `yes, but'), but yielded mixed results with the QUD-diagnostic.
...
\begin{itemize}[leftmargin=12pt]
  ...
  \item \sout{At-issueness is a key concept in experimental and theoretical semantics and pragmatics, relied on in the analysis of a broad range of phenomena (give examples). }

  \item \sout{But what is at-issueness? There are multiple definitions and multiple diagnostics that make different assumptions about how at-issue content differs from not-at-issue content (give details on definitions and diagnostics, these could be the diagnostics we will eventually use).}

  \item \sout{So, we're currently in a state where there is no agreed on definition of at-issueness and multiple diagnostics. This raises the question of whether the currently available definitions define the same underlying concept and whether the diagnostics yield the same result. These are pressing questions for research on at-issueness (see, e.g., Tonhauser et al 2018, Koev 2018, who else?)}

  \item \sout{Some research has suggested that the answers to both of these questions is ``no'':}

  \begin{itemize}

    \item \sout{On the theory-side, Snider: suggests that q-at-issueness and p-at-issueness differ (Korotkova?)}

    \item \sout{On the diagnostics-side, NRRCs: some research assumes that the content of medial and final ones is not-at-issue but Syrett/Koev results suggest that medial ones are more not-at-issue than final ones}

    \item \sout{On the diagnostics-side, Tonhauser et al 2018: comparison of asking-whether and are you sure diagnostic suggests some differences (p.526ff)}
...
\section{Experiments 1-4 \label{sec:2_experiments}}
  ...
  % To be able to compare the results of the diagnostics, the same seven kinds of contents, shown in \ref{stims}, were investigated under the four diagnostics:
  ...
  % instantiated by the same items across the four experiments.
  %
