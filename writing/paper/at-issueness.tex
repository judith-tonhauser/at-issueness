...
\section{Experiments 5-6 \label{sec:3_more-experiments}}
  ...
  \subsection{Discussion}
    ...
    The `asking whether' and direct response diagnostic, as implemented in Exps.~5 and 6, lead to results that have a high Spearman rank correclation, similar range of means, and high overlap in the by-predicate differences that come out as significant. The similar results between these two experiments, which both show high differentiation between different contents, suggests that this is due to interrogative embedding and not the response task.


    the speech act in which the target content is embedded makes a difference
...
\section{General discussion \label{sec:4_discussion}}
  ...
  \subsection{Diagnostic differences and notions of at-issueness}
    First, we aimed to determine the extent of consistency or divergence across these diagnostics when applied to identical linguistic stimuli. 
  ...
  \subsection{Speech act, commitment, and at-issueness}
    %
      Question-based vs. assertion-based at-issueness diagnostics: The question-based diagnostics (QUD, ‘asking whether’) are about whether a proposition is at issue relative to a QUD introduced in the previous discourse, wheras assertion-based diagnostics (direct dissent, ‘yes but’) target a different underlying notion.
    %
      We found that it makes a difference what speech act the tested content is in.
      %
      % This can be related to theoretical notions of QUD-based at-issueness definitions, and questions about what happens in assertion-based diagnostics.
      %
      But why might a question embedding lead to higher differentiation than assertions? Previous literature distinguishes between question-based diagnostics (QUD, ‘asking whether’), which are about whether a proposition is at issue relative to a QUD introduced in the previous discourse, wheras assertion-based diagnostics (direct dissent, ‘yes but’) target a different underlying notion.


      \begin{itemize}
        \item former at-issue content determining or addressing a QUD
        \item latter, at-issue content at-issue content proposal 
      \end{itemize}
      ...
      We hypothesize that this is because of the following:
      (i) when propositional content is not-at-issue, it projects, and we understand the speaker to be committed to it
      (ii) in assertions, but not questions, the speaker is also committed to the at-issue content
      (iii) it is easier for participants to distinguish not-at-issue conent and at-issue content when the speaker is not committed to the at-issue content, which would make the two more similar.
