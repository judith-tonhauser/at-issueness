...
The assumptions underlying these diagnostics can be organized around three properties of at-issue content that \citet{tonhauser_diagnosing_2012} identifies as motivating diagnostic strategies:
...
\footnotetext{
  Relevance to the QUD is defined as follows: (ref büring, see koev 2018)
...
\noindent This QUD-based definition aligns naturally with property (i), that at-issue content addresses the QUD. but there is no consensus about whether all of the above diagnostics and properties align with the notion of at-issueness defined in \ref{def:qai}. However, it is less clear whether all diagnostics align with this definition. In particular, several authors have argued that diagnostics targeting property (iii), namely the direct-dissent and `yes but' diagnostic, pattern differently from QUD-based diagnostics.

Empirically, this divergence has been prominently discussed in the literature on sentence-medial and sentence-final appositive NRRCs. While appositive NRRCs are widely taken to contribute not-at-issue content (\citealt{potts_logic_2005,amaral_review_2007,tonhauser_diagnosing_2012,destruel_cross-linguistic_2015,tonhauser_how_2018,solstad_cataphoric_2024}), a forced-choice continuation study by \citet{syrett_experimental_2015} found that sentence-final appositive NRRCs pattern as more at-issue than medial ones, under a version of the direct-dissent diagnostic. Based on intuitive judgment contrasts, \citet{snider_anaphoric_2017} argues that the direct-dissent diagnostic is sensitive to this medial/final distinction, but diagnostics targeting properties (i) or (ii) are not. Similar diagnostic differences have been reported for sentence-final slifting parentheticals (\citealt{koev_notions_2018}) and evidential inferences (\citealt{korotkova_evidential_2020}).

These reports of diagnostic differences have motivated the view that at-issueness is not a uniform notion (\citealt{koev_notions_2018}). In particular, property (iii) has been associated with a distinct, assertion-based notion of at-issueness, based on the view that assertions function as proposals to update the common ground (based on \citealt{stalnaker_assertion_1978,clark_contributing_1989,ginzburg_resolving_1995,farkas_reacting_2010}), under which the at-issue content of an assertion is the proposition proposed to be added to the common ground, whereas not-at-issue content is either presupposed (already entailed in the common ground; \citealt{stalnaker_presuppositions_1973,stalnaker_common_2002}), or newly imposed on the common ground (\citealt{murray_evidentiality_2010,murray_varieties_2014,anderbois_crossing_2010,anderbois_at-issue_2015}). On this view, at-issueness is defined as follows:\footnote{Other theoretical notions of at-issueness proposed in the literature include \citeauthor{esipova_not-at-issueness_2021}'s (\citeyear{esipova_composition_2019,esipova_not-at-issueness_2021}) analysis of at-issue content as involving restrictive modification and not-at-issue content as involving non-restrictive modification; as well as an understanding of at-issueness relative to coherence-based discourse structure (\citealt{hunter_shapes_2016,jasinskaja_not_2016}). It would be interesting to investigate the theoretical relationship between the various proposed definitions of at-issueness, to find out whether they are equivalent, orthogonal, or truly make different predictions.}

\ex. Proposal at-issueness: \hfill (\citealt{koev_apposition_2013,koev_notions_2018})\\
...
% This definition aligns theoretically with analyses of not-at-issue content in declarative assertions, that analyzes the not-at-issue content of presuppositions, evidentials (murray), and appositive NRRCs (anderbois);

However, alternative interpretations of the diagnostic differences have also been advanced. \citealt{anderbois_at-issue_2015} argue that QUD-based and proposal-based notions of at-issueness are not theoretically distinct: building on \citet{farkas_reacting_2010}, they argue that proposing content for the common ground is intrinsically linked to managing the QUD, as evidenced by parallels between assertions and polar questions. \citealt{faller_discourse_2019}, similarly, suggest that the dissent/assent of property (iii) can be understood as linked to answering the QUD.\footnote{
  Faller's work on evidentials (\citealt{faller_semantics_2002,faller_evidentiality_2006,faller_discourse_2019}) highlights that the reported content can be at-issue while crucially lacking speaker commitment, thus not being asserted. While Faller takes this as an argument for (unified) a QUD-based rather than assertion-based definition of at-issueness, her analysis does assume at-issue proposals that need not be asserted (c.f. phenomena that gave rise to analysis involving weakened and modified commitment in malamud...?).
}

A different line of argument, developed by \citet{snider_anaphoric_2017,snider_at-issueness_2017,snider_distinguishing_2018,korotkova_evidential_2020}, holds that diagnostics targeting property (iii) do not diagnose at-issueness at all. Instead, they are argued to track the availability of the target content for propositional anaphora such as response particles (\emph{yes/no}) or propositional proforms (e.g., \emph{that} in \emph{that's not true}). On this view, what makes dissent in these diagnostics appear \enquote{direct} is the availability of a suitable anaphoric antecedent, rather than the at-issue status of the content.


Finally, the tension between diagnostics targeting QUD-based at-issueness property (i) and the assertion-based at-issueness property (ii), also raises the question about the status of diagnostics targeting property (ii), such as the `asking whether' diagnostic. Following \citet{snider_anaphoric_2017}, property (ii) can be understood as a forward-looking counterpart of property (i): while property (i) concerns answering an established QUD, property (ii) concerns determining which alternatives a question introduces. Under standard assumptions that explicit questions introduce or update the QUD (\citealt{roberts_information_1996,ginzburg_interrogatives_1996}), property (ii) aligns naturally with the QUD-based definition in \ref{def:qai}, but is incompatible with assertion-based definitions, which apply only to declaratives.
%
% In fact, this connection can be made, with two assumptions: the question partition $q$ of an explicit question can set up a QUD (as is commonly assumed in \citealt{ginzburg_interrogatives_1996,roberts_information_1996}), and the context $c$ in the definition in \ref{def:qai} can be taken to be the context after the question is asked. Then, the at-issue content of a question in a context $c$ can be straightforwardly defined as in \ref{qai}, as the question meaning $q$ s.t. the speaker has a recognizable intention to set up the QUD via $q$, which therefore also is relevant to the QUD. For more systematic treatments of backward-looking versus forward-looking QUD-relevance and QUD-at-issueness, see also accounts of dynamic update to the QUD, Robert's QUD stacks, Büring's discourse trees, and Oneas current question, (... hernot mortier).

In sum, existing diagnostics differ both in the properties they target and in the theoretical notions they are taken to diagnose. Table~\ref{tab:diagnostics-notions} summarizes how the four diagnostics discussed here have been mapped to different underlying notions of at-issueness in the literature: \citealt{tonhauser_diagnosing_2012} assumes that the three properties targeted by at-issueness diagnostics all reflect QUD at-issueness, in line with theoretical assumptions that the phenomenon targeted by this definition is not distinct from what is targeted by the proposal-based definition (\citealt{anderbois_at-issue_2015,faller_discourse_2019}). \citet{snider_anaphoric_2017}, likewise, assumes a uniform underlying notion of at-issueness (relative to the QUD), while arguing that diagnostics targeting property (iii) do not diagnose at-issueness of content, but its availability for propositional anaphora. \citealt{koev_notions_2018}, in contrast, assumes that QUD at-issueness and proposal at-issueness are distinct underlying phenomena corresponding to two different notions of at-issueness.


\begin{table}[t]
\centering
\resizebox{\linewidth}{!}{\begin{tabular}{l p{3.1cm} p{3.2cm} p{2.3cm} p{1.9cm}}
\toprule
...
\multirow{2}{*}{(1) QUD diagnostic}
 ...
 & \multirow{7}{*}{\parbox{3.2cm}{\centering QUD at-issueness\\ (/proposal\\ at-issueness)}}
...
To date there has been no systematic experimental comparison of diagnostics to investigate whether the diagnostics yield consistent results; and there is currently no agreed-upon definition of at-issueness, and existing diagnostics may not be probing the same underlying phenomenon. This raises two central questions:
...
Given that \citealt{syrett_experimental_2015} have found positional effects for apposistive NRRCs under a version of the direct-dissent diagnostic. 
...
Since \citet{snider_at-issueness_2017} has argued that the direct-dissent diagnostic is sensitive to this medial/final distinction, while other tests are not, the question arises whether it can be found across different diagnostics in a systematic experimental comparison.
