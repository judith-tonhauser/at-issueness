...
length(unique(cd$participant_id)) #3 participants
...
cols = data.frame(V=levels(prop$expression))
...
cols$V <- factor(cols$V, levels = cols[order(as.character(prop$expression)),]$V, ordered = TRUE)
...
subjmeans = cd %>%
  group_by(expression,participant_id) %>%
  summarize(Mean = mean(nResponse)) 
subjmeans$expression <- factor(subjmeans$expression, levels = unique(levels(prop$expression)))
...
ggplot(prop, aes(x=expression, y=Mean)) +
  ...
  ylab("Proportion of 'no' (at-issueness) selections") +
  xlab(" ") +
  theme(axis.text.x = element_text(size = 12, angle = 45, hjust = 1)) 
ggsave("../graphs/YB.pdf",height=4.5,width=7)
