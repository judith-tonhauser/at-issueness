...
names(shapes) <- levels(qud_yb$expression)

# Define a vector of colors for 7 expressions
colors <- c("darkorchid", "black", "gray60", "tomato1", "dodgerblue", "forestgreen", "gold")
names(colors) <- levels(qud_yb$expression)

pp <- ggplot(qud_yb, aes(x = Mean_qud, y = Mean_yb, fill = expression, shape = expression)) +
...
pp
ggsave("../graphs/YBvsQUD.pdf", height = 5, width = 5)
