...
We hypothesize that the low ratings in Exp.~1 arise because utterances which include \emph{be right} in a veridical environment, like Leo's utterance in \ref{beright.a}, convey that the attitude holder (here, Danny) has publicly committed to the content of the clausal complement (here, that Lucy broke the plate).\footnote{\citet{abusch_lexical_2002,abusch_presupposition_2010} characterizes \emph{be right} as conveying a doxastic inference that the subject believes the embedded proposition---an inference that is empirically characterized as projective in (\citealp[p.~50]{abusch_presupposition_2010}),
% \citet[p.~6--8]{abusch_lexical_2002} derives projection for this inference via reasoning over lexically determined alternatives.
Following a suggestion in \citealt{schlenker_local_2010} (p. 135), \citet[p.~73]{anand_factivity_2014} argue that the projective inference of \emph{be right} is assertive rather than doxastic, and conveys that the subject has committed to the embedded content by means of a prior discourse move.
...
We can understand the ill-formedness of \Last with a QUD-based framework of discourse structure (in the sense of \citealt{ginzburg_interrogatives_1996,roberts_information_2012,buring_d-trees_2003}). Within this framework, Danny's assertion is assumed to address the question whether Lucy broke the plate.\footnote{
