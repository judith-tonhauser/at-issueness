...
In contrast, views that take different diagnostics to probe distinct phenomena are motivated by empirical divergences between them. For example, although appositive NRRCs are widely taken to contribute not-at-issue content, a forced-choice continuation study by \citet{syrett_experimental_2015} found\jt{past tense ``found'' here, but the manuscript also uses a lot of present tense for other works, e.g., ``suggests'' or ``takes''. I don't care whether we use past tense or present tense, but we should be consistent. please change all discussion of prior work to either past or present tense consistently} that sentence-final appositive NRRCs pattern as more at-issue than medial ones, under a version of the direct-dissent diagnostic, which targets property \ref{properties.iii}. \citet{snider_anaphoric_2017} argues that diagnostics targeting property \ref{properties.iii} are sensitive to this medial vs.~final distinction, but diagnostics targeting properties \ref{properties.i} or \ref{properties.ii} are not. Similar diagnostic differences have been reported for sentence-final slifting parentheticals (\citealt{koev_notions_2018}) and evidential inferences (\citealt{korotkova_evidential_2020}).

Such differences have been interpreted in two main ways. \citet{koev_notions_2018} takes them to indicate that at-issueness is not a uniform notion. On this view, the QUD-diagnostic (based on property \ref{properties.i}) targets a QUD-based notion of at-issueness, while the direct-dissent and `yes but' diagnostics (based on property \ref{properties.iii}) probe a proposal-based notion. The two definitions are thus taken to characterize distinct discourse phenomena, which may diverge empirically.
%
Another view (\citealt{snider_anaphoric_2017,snider_at-issueness_2017,snider_distinguishing_2018,korotkova_evidential_2020}) maintains a unified notion of at-issueness, arguing that diagnostics targeting properties (i) and (ii) genuinely probe at-issueness, whereas property-(iii) are instead sensitive to the availability of the target content for propositional anaphora. Property-(iii) diagnostics are thus taken to reflect whether the target content can provide antecedents for response particles (\emph{yes/no}) or propositional pro-forms (e.g., \emph{that} in \emph{that's not true}), which are present in the direct-dissent and `yes but' diagnostics.

\jt{The next paragraph is an alternative formulation for the preceding paragraph: I don't understand why the second view is characterized as ``unified'', if S and K essentially say that diagnostics based on property iii do not diagnose at-issueness. Is ``unified'' the best way to describe this view? Is it necessary to give a name to this view?}

Such observation have been interpreted to mean that the different diagnostics do not diagnose the same underlying concept. For \citet{koev_notions_2018},  the QUD-based notion of at-issueness and the proposal-based notion of at-issueness are distinct notions, and the corresponding diagnostics therefore diagnose distinct discourse phenomena. Other works (\citealt{snider_anaphoric_2017,snider_at-issueness_2017,snider_distinguishing_2018,korotkova_evidential_2020}) maintain that diagnostics targeting properties \ref{properties.i} and \ref{properties.ii}  probe at-issueness, whereas diagnostics targeting property \ref{properties.iii} are instead sensitive to the availability of the target content for propositional anaphora. These latter diagnostics are  taken to reflect whether the target content can provide antecedents for response particles (\emph{yes/no}) or propositional pro-forms (e.g., \emph{that} in \emph{that's not true}), which are present in the direct-dissent and `yes but' diagnostics. \jt{end alternative paragraph}
...
The `are you sure?' diagnostic presents participants with dialogues like \Next and collects ratings about whether the response addresses the question (c.f. the `really' test of \citealt{shanon_two_1976}).\jt{do you use ``cf.'' to mean ``see'' or ``contrasting with''? given that you also use ``see'' I assume the latter but is that correct here (and with the cf. you have above)?}
...
Subsequent work shows that the `asking whether' diagnostic can detect fine-grained differences. \citet[Exp.~1]{degen_projection_2025} found fine-grained at-issuneess differences among the contents of the complements of 20 clause-embedding predicates in English. Fig.~\ref{fig:dtglossa} shows the mean `asking whether' ratings for the 20 predicates used in their study, ordered by mean rating. \jt{Does the repo contain the code needed to create this figure?}
\begin{figure}[h]
  \centering
  \includegraphics[width=0.7\textwidth]{../../results+analysis/degen-tonhauser-glossa/graphs/mean-asking-whether-ratings.pdf}
  
  \caption{Mean `asking whether' ratings for the contents of the clausal complements of 20 clause-embedding predicates, based on \citealt[Exp.~1]{degen_projection_2025}. Error bars indicate 95\% bootstrapped confidence intervals. Violin plots indicate the distribution of individual ratings.
  }
  \label{fig:dtglossa}
\end{figure}

\jt{Does this violin plots not indicate ``the kernel probability density of individual participants' ratings'', but just ``the distribution of individual ratings''?}
...
%\begin{enumerate}
%  \item Do different diagnostics yield consistent results when applied to the same contents?
%  \item Do different diagnostics reflect distinct underlying notions of at-issueness?
%\end{enumerate}
...
Each experiment tested the same set of contents associated with the seven types of expressions in \ref{stims}: the contents of sentence-medial and sentence-final NRRCs \Next[a--b], as well as the contents of the clausal complements of \emph{be right, discover, confess, confirm, discover,} and \emph{know} \Next[c--g]. \jt{Why were these five predicates chosen? This needs to be related to Fig. 1}
...
\jt{The y-axis of Exp 4 states "higher response = more at-issue", which is odd, given that the responses are categorical. Please change to something like "higher proportion".}
...
The remainder of this section discusses this result and also addresses the second research question in \ref{rqb}. Specifically, Section \ref{ssec:discussion-methods} discusses a methodological implications of the observation that most of the diagnostics did not yield consistent results when applied to the same contents. Section \ref{ssec:discussion-questions} develops a hypothesis for why the two diagnostics in which the target contents were embedded in polar questions yielded greater by-content differentiation than the diagnostics in which the contents were embedded in declaratives. Finally, Section \ref{ssec:discussion-differences} addresses research question \ref{rqb}.
