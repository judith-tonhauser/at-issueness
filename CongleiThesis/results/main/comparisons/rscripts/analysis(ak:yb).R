...
names(shapes) <- levels(ak_yb$expression)

# Define a vector of colors for 7 expressions
colors <- c("darkorchid", "black", "gray60", "tomato1", "dodgerblue", "forestgreen", "gold")
names(colors) <- levels(ak_yb$expression)

pp <- ggplot(ak_yb, aes(x = Mean_ak, y = Mean_yb, fill = expression, shape = expression)) +
  ...
  xlab("Mean 'askng whether' (at-issueness) ratings") +
...
pp
ggsave("../graphs/YBvsAK.pdf", height = 6, width = 6)
