...
length(unique(cd$participantID)) #75 participants

# mean at-issueness by target expressions, including the controls
means = cd %>%
  group_by(expression) %>%
...
ggplot(means, aes(x=expression, y=Mean)) +
 ...
 # scale_fill_manual(values=rev(c("#D55E00","#009E73","black")),labels=rev(c("factive","nonfactive","main clause\ncontrols")),name="Predicate type") +
  ...
  ylab("Mean QUD (at-issueness) ratings") +
  xlab(" ") +
  theme(axis.text.x = element_text(size = 12, angle = 45, hjust = 1)) 
ggsave("../graphs/mean-QUD-ratings.pdf",height=4.5,width=7)
