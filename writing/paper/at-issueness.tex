...
\bigskip \large
Word count (including references, excluding abstract and supplements): 10,338
}
...
%TC:ignore
\begin{abstract}
At-issueness is a key concept in semantics and pragmatics, used to distinguish the main point of an utterance from background information. However, there is no consensus on how at-issueness should be defined, and different diagnostics reflect different assumptions about its underlying nature. This paper presents a systematic experimental comparison of four commonly used at-issueness diagnostics, applied to the same target contents, including appositive non-restrictive relative clauses and the complements of clause-embedding predicates. The results show that the diagnostics yield divergent results for some contents, with only one diagnostic reproducing previously reported differences between contents. Further comparison suggests that this diagnostic's greater sensitivity to by-content differences is driven by the use of polar questions to present target content, rather than by the response task itself. These findings suggest that the speech act used to present target content plays an important role in at-issueness diagnostics, and raise questions about whether different diagnostics reflect distinct underlying notions of at-issueness or different perspectives on a single phenomenon.
\end{abstract}
\begin{keywords}
...
Diagnostics of at-issueness operationalize these theoretical notions via linking assumptions about how at-issue content behaves in discourse. These can be organized around three properties of at-issue content identified in \citet{tonhauser_diagnosing_2012} as motivating diagnostic strategies:
...
The question whether different definitions of at-issueness capture a single notion or distinct ones is reflected in competing views about whether diagnostics probe distinct underlying phenomena or provide different ways of probing a single, unified phenomenon.
...
Such differences have been interpreted in two main ways. \citet{koev_notions_2018} takes them to indicate that at-issueness is not a uniform notion. On this view, the QUD-diagnostic (property i) targets a QUD-based notion of at-issueness, whereas the direct-dissent and `yes but' diagnostics (property iii) probe a proposal-based notion. The two definitions are thus taken to characterize distinct discourse phenomena, which may diverge empirically.
%
...
while diagnostics targeting property (iii) are sensitive to a different phenomenon, namely the availability of the target content for propositional anaphora. Property-(iii) diagnostics are thus taken to reflect whether the target content can provide antecedents for response particles (\emph{yes/no}) or propositional pro-forms (e.g., \emph{that} in \emph{that's not true}), which are present in the direct-dissent and `yes but' diagnostics.
...
Initial experimental work on these questions has yielded mixed answers. As noted above, \citet{syrett_experimental_2015} reported a positional effect for appositives under a version of the direct-dissent diagnostic. In their Experiment~2, participants saw a declarative sentence introducing the target content and chose between two \emph{no}-responses expressing direct dissent, targeting either the appositive or the main-clause content \Next.

\ex. \citealt{syrett_experimental_2015}, p.~17
\a.[A:] My friend Sophie, a classical violinist, performed a piece by Mozart.
\b.[B1:] No, she's not. \hfill (target: NRRC)
\b.[B2:] No, she didn't. \hfill (target: main clause)
\z. \z. 

By assumption, direct dissent targeting the at-issue main clause is always available. In contrast, the B1 response is expected to be strongly dispreferred when the target content is not-at-issue, as for the NRRC content in \Last. Consistent with this expectation, participants strongly preferred B2 responses (with proportions around 79\%) when the competing alternative targeted a sentence-medial NRRC. Crucially, this preference was weaker for sentence-final NRRCs (around 65\%), suggesting that final NRRCs may be interpreted as more at-issue than medial ones. Since this positional contrast for NRRCs has been argued to be specific to assent and dissent-based (property iii) diagnostics, a key question is whether it also emerges with other diagnostics.
...
The `are you sure?' diagnostic is taken to target property (iii), by testing whether the target content can be directly challenged. The target content is introduced by the initial assertion, the question challenges the assertion, and the response reinforces the target content. The response is expected to be judged as addressing the question for at-issue content, and as not addressing the question otherwise. Accordingly, the diagnostic is expected to yield low ratings for the NRRC in \Last.

Comparing the diagnostics (p. 526ff.), the authors found both convergence and divergence. Ratings were correlated across diagnostics, suggesting that they may be sensitive to a shared underlying phenomenon. However, the `asking whether' diagnostic yielded lower overall ratings and smaller differentiation between contents than the `are you sure?' diagnostic.
...
The possibility that diagnostics may differ in how strongly they differentiate among contents raises another key question: whether different diagnostics systematically reproduce fine-grained by-content differences for clause-embedding predicates.

\subsection{Outlook}
This paper presents a systematic experimental comparison of at-issueness diagnostics across six experiments, taking a first step towards addressing the two overarching research questions: (i) whether at-issueness diagnostics yield consistent results when applied to the same target contents, and (ii) whether they reflect distinct underlying notions of at-issueness.

To address (i), we compare diagnostics by testing whether they reproduce by-content contrasts reported in prior work. Specifically, we examine whether they converge on the positional effect for appositive NRRCs and fine-grained differences among clause-embedding predicates.

To begin addressing (ii), we consider how any observed differences should be explained. Convergent results would support a unified notion of at-issueness, and divergence would raise the question whether it aligns with existing distinctions, such as QUD-based vs. proposal-based at-issueness or at-issueness vs. anaphoric availability, or whether it calls for an alternative explanation.

In what follows, \Cref{sec:2_experiments} reports on Experiments~1--4, which apply the four diagnostics in \ref{qud}--\ref{yesbut} to seven contents in English, including medial and final appositive NRRCs and the complements of selected clause-embedding predicates from \citet{degen_projection_2025} (\emph{confess, confirm, discover, know, be right}). None of these four experiments reproduced the positional effect for NRRCs reported by \citet{syrett_experimental_2015}. Only Exp.~2 (`asking whether') came close to observing the by-predicate differences found in \citet{degen_projection_2025}, where complements of \emph{know} were rated as less at-issue than those of \emph{discover}, followed by \emph{confess}, then \emph{confirm}, and then \emph{be right}. This suggests that diagnostics do vary in whether they detect at-issueness differences between particular expressions. Notably, the `asking whether' diagnostic, which showed the greatest differentiation between contents in Exp.~2, is the only diagnostic which asked participants to judge the intentions of the speaker directly, and the only one where the target content is embedded in a polar question.

\Cref{sec:3_more-experiments} asks how this observed difference should be explained, investigating whether the greater differentiation under the `asking whether' diagnostic was due to the polar question embedding or the response task. Experiments~5 and 6 compared the `asking whether' diagnostic with a direct-response diagnostic (\citealt{amaral_review_2007,tonhauser_diagnosing_2012}) that also embeds content in a polar question but uses a different response task, eliciting acceptability judgments. Using the 20 clause-embedding predicates from \citet{degen_projection_2025}, the two experiments yielded highly correlated results, indicating that the observed high differentiation in Exp.~2 was driven by the polar question embedding, not the response task.

The clearest difference between diagnostics that we found is, therefore, between diagnostics that embed the target content in polar questions, such as the `asking whether' diagnostic (Exps.~2, 5) and the direct-response diagnostic in (Exp.~6), which found higher differentiation between contents than diagnostics that embed the target content in assertions, such as the direct-dissent and `yes, but' diagnostics (Exps.~3--4). This pattern is not predicted by a dual-notion view of at-issueness, such as the distinction between QUD-based and proposal-based notions, or by the distinction between at-issueness and anaphoric availability. Instead, our results provide evidence that the speech act used to present the target content plays an important role: embedding the target content in polar questions yield results with greater by-content differentiation than embedding it in assertions. \Cref{sec:4_discussion} provides further discussion of these points, and \Cref{sec:5_conclusion} concludes.
...
As shown in Fig.~\ref{fig:results}, Exp.~2 (`asking whether') largely reproduces the pattern from \citet{degen_projection_2025}, namely, that complements of \emph{know} were rated as less at-issue than those of \emph{discover}, followed by \emph{confess}, then \emph{confirm}, and finally \emph{be right}, with the only exception that \emph{confess} and \emph{discover} did not differ in our experiment.
...
We hypothesize that the low ratings in Exp.~1 arise because \emph{be right} imposes a contextual felicity requirement that a prior commitment by an attitude holder be salient in the discourse. Utterances of the form \emph{X is right that $p$} require that the common ground or discourse model contains the information that $X$ has previously endorsed $p$ (or a closely related proposition) (\citealt{abusch_presupposition_2010}, p. 50; \citealt{anand_factivity_2014}).
% \footnote{This contextual requirement of \emph{be right that $p$}, that there be a salient prior commitment may be a reason for the common use of \emph{right} and related items in agreeing responses (see, e.g, \citealt{krifka_response_2013,greenberg_intensified_2019,oshima_semantics_2026}).}
...
To distinguish between these possibilities, Exps.~5--6., reported in the next section, compare the `asking whether' diagnostic to a diagnostic that also embeds the target content in a polar question but elicits naturalness ratings to assess at-issueness. Both experiments yielded similarly fine-grained distinctions among contents, supporting the conclusion that the increased differentiation observed in Exp.~2 is driven primarily by interrogative embedding rather than by the response task.
...
We excluded the data of participants who did not self-identify as native speakers of American English, and of participants whose responses to one of the two control trials was more than 2 sd away from the group mean. Table \ref{t:excluded2} shows how many participants were excluded in each experiment, the properties of the remaining participants, and the number of data points that entered into the analyses.
...
\lh{Okay, I am leaving this here in a subsection for now. Maybe you would like to add something, or do you think that this merits its own subsection as it is? Alternatively, it could be used above, before the paragraph that starts \enquote{The remainder of this section\dots}?}
