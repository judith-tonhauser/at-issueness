...
\section{Introduction \label{sec:1_introduction}}
  ...
  Overall, our findings show that the four diagnostics are not interchangeable: they differ in how strongly they differentiate among the contents tested. While our results do not conclusively establish whether all differences stem from the response tasks, from distinct underlying notions of at-issueness, or from other discourse factors, we do find evidence that the speech act used to present the target content matters. A key takeaway is that future empirical work on at-issueness should continue to employ multiple diagnostics, to avoid results that are tied to a single diagnostics. Moreover, because the interrogative embedding appears to play an important role, speech act type should be taken into account when diagnosing at-issueness.
...
\section{Experiments 1-4 \label{sec:2_experiments}}
  ...
  \subsection{Discussion}
    ...
    \subsubsection{Do ranking differences suggest different underlying notions? \label{ssub:be-right}}
      ...
      As a result, low QUD-match ratings for \Last[a] arise not because the embedded clause fails to be at-issue, but because the utterance presupposes an incoherent discourse context.%
      ...
      Ultimately, whether the by-content rank differences in Exps.~1--4 provide evidence for distinct underlying notions of at-issueness remains inconclusive, while the different behaviors found for \emph{be right} show that diagnostics may interact differently with the contextual requirements of the target contents.
...
\section{Experiments 5 and 6 \label{sec:3_more-experiments}}
  %
    ...
    Question-Answer direct response diagnostic
...
\section*{Ethics and consent (if applicable)}
  ...
  Research involving human subjects, human material, or human data, must have been performed in accordance with the Declaration of Helsinki. Studies must have been approved by an appropriate ethics committee and the authors should include a statement in the article text detailing this approval, including the name of the ethics committee and reference number of the approval, or mention any exemptions to ethical approval that apply to their research. The identity of research subjects should be anonymized whenever possible. For research involving human subjects, informed consent to participate in the study must be obtained from participants (or their legal guardian).
