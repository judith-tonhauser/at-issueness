...
\noindent Another prominent definition, given in \Next, is grounded in understanding an assertion as a proposal to update the common ground (\citealt{stalnaker_assertion_1978,clark_contributing_1989,ginzburg_resolving_1995,farkas_reacting_2010}). Here, the at-issue content of an assertion is defined as the content that is proposed for addition to the common ground (\citealt{koev_apposition_2013}), while not-at-issue content is presupposed (already entailed in the common ground; \citealt{stalnaker_presuppositions_1973,stalnaker_common_2002}), or added to the common ground without being proposed (\citealt{murray_evidentiality_2010,murray_varieties_2014,anderbois_crossing_2010,anderbois_at-issue_2015}).
...
Four commonly used diagnostics that build on the properties in \Last are illustrated in (\pref{qud}--\pref{yesbut}) for sentence-medial non-restrictive relative clauses (NRRCs), which are typically taken to contribute not-at-issue content (\citealt{potts_logic_2005}).
...
Subsequent work shows that the `asking whether' diagnostic can detect fine-grained differences. \citet[Exp.~1]{degen_projection_2025} found fine-grained at-issueness differences among the contents of the complements of 20 clause-embedding predicates in English. Fig.~\ref{fig:dtglossa} shows the mean `asking whether' ratings for the 20 predicates used in their study, ordered by mean rating. 
\begin{figure}[h]
  \centering
...
%The clearest difference we found is therefore between diagnostics that embed the target content in polar questions and those that embed it in declaratives. Diagnostics with polar question embedding, like the `asking whether' diagnostic (Exps.~2, 5) and the direct-response diagnostic in (Exp.~6), show greater differentiation between contents than diagnostics with declarative embeddings, like the direct-dissent and `yes, but' diagnostics (Exps.~3--4). This pattern is not predicted by a dual-notion view of at-issueness (QUD-based vs. proposal-based)\jt{The term ``dual-notion view of at-issueness'' has not been properly introduced. I am also worried about attributing it to Koev 2018, since that work discusses three notions of at-issueess.} or by the distinction between at-issueness and anaphoric availability. Instead, we 
...
Each experiment tested the same set of contents associated with the seven types of expressions in \ref{stims}: the contents of sentence-medial and sentence-final NRRCs \Next[a--b], as well as the contents of the clausal complements of \emph{be right, discover, confess, confirm, discover,} and \emph{know} \Next[c--g].
...
\jt{In Fig3/Exp 2 the expressions are ordered from left to right by increasing at-issueness, but the expressions are ordered from left to right by decreasing at-issueness in the four panels of Fig 4. This needs to be adjusted, either in Fig 3 or in Fig 4. I vote for Fig 3 because those changes and figures are way easier to make (and isn't that the order in which things were originally? why was it changed?).}
...
One factor that might explain the difference between our findings and those of \citet{syrett_experimental_2015}, aside from the stimuli, is the response task. Although both studies used dissent-based diagnostics with target content presented in assertions, \citepos{syrett_experimental_2015} forced-choice implementation differs from our implementation of the dissent-based diagnostics in Exps.~3 and 4.

In Exp.~3 (direct dissent), we elicited naturalness ratings rather than forced-choice judgments. In Exp.~4 (`yes, but'), which did use a forced-choice continuation task, the response options were different from those used by Syrett \& Koev. In our implementation (based on \citealt{xue_correlation_2011}), all options rejected the target content, and participants chose between expressing direct dissent (\emph{no}), or indirect dissent (with \emph{yes, but} or \emph{yes, and}). In contrast, in Syrett \& Koev's study, all options expressed direct dissent (with \emph{no}), and participants chose whether to reject the NRRC content or the main clause content.

The results suggest that positional effects for NRRCs are not robust across diagnostics and, furthermore, the corresponding task differences suggest that even small changes in the response task used to operationalize a diagnostic, such forced-choice alternatives, could reverse the observed pattern. More generally, different diagnostics vary in whether they detect any positional difference for NRRCs at all. This suggests that previously reported effects may reflect task- or diagnostic-specific effects rather than stable properties of NRRC interpretation. A natural follow-up would be to test whether applying their particular forced-choice schema to our stimuli would reproduce their effect.
...
The diagnostics, as implemented here, yield different rank orderings of contents. The most pronounced ranking difference concerns the complement of \emph{be right}: it receives the lowest ratings in Exp.~1 (QUD diagnostic)  and relatively high ratings in the other experiments.
...
\jt{I am uncomfortable with the notion of a `contextual felicity constraint' that you use in this subsection because  I do not understand what that is supposed to be. It's not a strong contextual felicity constraint because ``be right'' can be used out of the blue without problem; we just infer that the attitude holder claimed CC. I don't think we need to use this term to explain why ``be right'' received such low ratings in Exp 1. See below for a proposed replacement text from me}

The pattern for \emph{be right} could instead be explained by how the QUD diagnostic interacts with the discourse requirements of \emph{be right}. First consider that, as shown in panel (a) of Fig.~\ref{fig:results},  participants gave relatively low naturalness ratings to responses such as \ref{beright.a}, indicating that they did not judge the response as fitting the question.
...
We hypothesize that the low ratings in Exp.~1 arise because an utterance including \emph{be right} imposes a contextual felicity requirement that a prior commitment by an attitude holder be salient in the discourse. Utterances of the form \emph{X is right that $p$} require that the common ground or discourse model contains the information that $X$ has previously committed to $p$ (\citealt{abusch_presupposition_2010}, p. 50; \citealt{anand_factivity_2014}).\footnote{This contextual requirement of \emph{be right that $p$}, that there be a salient prior commitment may be a reason for the common use of \emph{right} and related items in agreeing responses (see, e.g, \citealt{krifka_response_2013,greenberg_intensified_2019,oshima_semantics_2026}).} In \ref{beright.a}, this requirement is that Danny has previously committed to the proposition that \enquote{Lucy broke the plate}, as would be the case in the discourse in \Next.

We hypothesize that the low ratings in Exp.~1 arise because utterances of positive declaratives with \emph{be right}, like Leo's utterance in \ref{beright.a}, convey that the attitude holder (here, Danny) is publicly committed to the content of the clausal complement (here, that Lucy broke the plate), as made explicit in \Next.\footnote{\jt{You referred to ``(\citealt{abusch_presupposition_2010}, p. 50; \citealt{anand_factivity_2014})'' for the claim that be right introduces a contextual requirement. I do not see that Abusch 2010 says anything of this sort on p.50 -- did you mean a different page or work? What's the relevant page number for Anand \& H 2014? I also do not see where they make this sort of claim; if they do, I'd like to refer to them in my paper with Noa.}} In the context of Danny's assertion, Leo's \emph{be right}-utterance is conveys that he considers Danny's speech act justified (see \citealt{krifka_response_2013} on the response particle \emph{right} and \citealt{attali_tale_2026} on the meaning of \emph{be right}).


\ex.  \a. {\bf Danny:} \emph{Lucy broke the plate.} \hfill (QUD: \enquote{whether Lucy broke the plate})
  \b. {\bf Nora:} \emph{What did Lucy break?}
  \b. {\bf Leo:} \emph{Danny is right that she broke the plate.} \z.
  
  \jt{Why is `Did Lucy break the plate?' the QUD for Danny's utterance? Aren't  `What did Lucy break?' or `Who broke the plate?' also possibilities?}

We can understand the ill-formedness of \Last within a QUD-based framework of discourse structure (in the sense of \citealt{ginzburg_interrogatives_1996,roberts_information_2012,buring_d-trees_2003}). In this framework, the contextual requirement of \emph{be right}, that a prior commitment be salient in the discourse, will result in a requirement that the QUD addressed by the act of making this commitment addresses must be part of the QUD stack. Specifically, in \Last, Danny's commitment that \enquote{Lucy broke the plate} addresses the QUD (or sub-QUD) \enquote{whether Lucy broke the plate}. This means that the question \enquote{Whether Lucy broke the plate} must be part of the QUD stack at the point where \emph{be right} is uttered.

This question is a subquestion of the question in \Last[b] \enquote{What did Lucy break}, since every complete answer to the latter entails an answer to the former. The progression from the contextually required subquestion to the explicitly given superquestion violates \citepos{roberts_information_2012} constraint on QUD stacks (p. 6:15), which allows only the reverse order (from superquestions to subquestions).\footnote{This constraint on QUD stacks can be understood as an informativeness-constraint, which requires that discourse moves contribute towards increasing the information quantity in the context set. Accordingly, the exchange in \Last is acceptable only under an echo-question interpretation of Nora's question.}

Under the hypothesis put forward here, the low ratings in Exp.~1 do not indicate that the embedded proposition is not at-issue.\footnote{\label{fn:w-o-be-right}
...
\begin{tabular}{l | c c c c}
  ...
  \hline
  % Exp.~4 (`yes, but' diagnostic) & & & & \cellcolor{lightgray} \\ \hline
\end{tabular}} Rather, they reflect that the response is infelicitous in the given discourse context. In the other diagnostics \ref{beright.b}--\ref{beright.d}, no conflicting prior discourse is given, allowing the discourse requirements of \emph{be right} to be accommodated without a reduction in acceptability. This highlights that diagnostics may interact with contextual requirements independently of at-issueness, and such interactions must be taken into account when interpreting the results.

\jt{Replacement text starts here.}
...
We can understand the ill-formedness of \Last with a QUD-based framework of discourse structure (in the sense of \citealt{ginzburg_interrogatives_1996,roberts_information_2012,buring_d-trees_2003}). Within this framework, Danny's assertion is assumed to address a QUD that is on the QUD stack, like the QUD \enquote{Whether Lucy broke the plate}. This QUD must be part of the QUD stack at the point where \emph{be right} is uttered. This QUD is a subquestion of Nora's question in \Last[b] \enquote{What did Lucy break}, since every complete answer to the latter entails an answer to the former. The progression from the subquestion to the explicitly given superquestion violates \citepos{roberts_information_2012} constraint on QUD stacks (p. 6:15), which allows only the reverse order (from superquestions to subquestions).\footnote{This constraint on QUD stacks can be understood as an informativeness-constraint, which requires that discourse moves contribute towards increasing the information quantity in the context set. Accordingly, the exchange in \Last is acceptable only under an echo-question interpretation of Nora's question.} In sum, the low ratings in Exp.~1 need not indicate that the embedded proposition is not at-issue but may be due to an infelicitous response in the given discourse context.\footnote{\label{fn:w-o-be-right}
When \emph{be right} is excluded, the Spearman rank correlations are:
...
\begin{tabular}{l | c c c c}
  ...
  \hline
  % Exp.~4 (`yes, but' diagnostic) & & & & \cellcolor{lightgray} \\ \hline
\end{tabular}} In the other diagnostics \ref{beright.b}--\ref{beright.d}, no prior discourse is given. This discussion highlights that diagnostics may interact with lexical meaning independently of at-issueness, and such interactions must be taken into account when interpreting the results.

\subsubsection{Why does the `asking whether' diagnostic show greater differentiation? \label{ssub:differentiation}}
A striking difference between the results of the experiments is that Exp.~2 (`asking whether') showed the greatest differentiation between contents, whereas the other three experiments exhibited a smaller range of means, and fewer statistically supported differences between investigated contents. Among the first four experiments, Exp.~2 was the only one that came close to differentiating the contents of the complements of the five clause-embedding predicates that were differentiated in \cite{degen_projection_2025}.

Let's consider whether this difference between diagnostics aligns with the divide emphasized in prior work between diagnostics that have been associated with a QUD-based definition (QUD, `asking whether' \jt{who associates the `asking whether' diagnostic with a QUD-based definition of at-issueness? not in Tonhauser et al 2018, not in the current paper}) and those that have been argued to target a proposal-based definition (direct-dissent, `yes, but'). However, this distinction could not explain why the `asking whether' diagnostic behaves differently from the QUD diagnostic, since both have been linked to the QUD-based definition of at-issueness.\jt{if we need to make the argument that the asking whether diagnostic is associated with the QUD-based definition of at-issueness because there is no prior literature that claims this, this might take us too far away from the point we're trying to make here. perhaps we can just omit this entire paragraph?}
...
%Both experiments yielded similarly fine-grained distinctions among contents, supporting the conclusion that the increased differentiation observed in Exp.~2 (`asking whether') is driven primarily by interrogative embedding rather than by the response task.
...
\\ {\bf Nora:} \emph{Is Tom right that Lucy broke the plate?}
\\ {\bf Leo:} \emph{Yes, she broke the plate.} \jt{I changed this from ``yes, she didn't break the plate'', please correct if needed}
...
\jt{predicates in Fig 6 are ordered from left to right by increasing at-issueness; the reverse is true for the predicates in Fig 7. this should be fixed. As above, I suggest adjusting fig 6 because that's easier than Fig 7}
...
%At the same time, not all differences between diagnostics are explained by this contrast. We also observed diagnostic-specific effects tied to particular contents and task designs. For example, we found that diagnostics differ in whether they detect positional effects for NRRCs, and even in the direction of these effects (\S~\ref{ssub:appositives}). Another case is the complement of \emph{be right}, which ranked low under the QUD diagnostic (Exp.~1) despite ranking high in the other experiments (\S~\ref{ssub:be-right}). These findings bear on the second research question. Empirical divergence between different diagnostics raises the possibility that they track distinct underlying discourse phenomena, such as QUD-based at-issueness, proposal-based at-issueness, or anaphoric availability. However, such an interpretation is only supported if the observed differences align systematically with the contrasts predicted by those theoretical views, which our results do not show. The clearest contrast we found, the difference between polar-question-based and assertion-based diagnostics, cuts across existing theoretical classifications. Other differences are more plausibly attributed to contextual requirements of particular expressions, or response-task differences. While this does not rule out the possibility that some diagnostics are sensitive to distinct underlying phenomena, but it does suggest that much of the observed variation arises from factors other than the theoretical distinctions currently discussed in the literature.

%The remainder of this section develops these points in more detail. \S~\ref{ssec:discussion-methods} discusses the methodological implications of diagnostic non-convergence. \S~\ref{ssec:discussion-questions} considers the role of question embedding and speaker commitments in yielding greater by-content differentiation. \S~\ref{ssec:discussion-differences} returns to the broader question of whether the observed differences support distinct underlying notions of at-issueness.
...
\item For NRRCs: Syrett/Koev found distinction, we did not. Extremely not-at-issue under asking-whether diagnostic, but not under the other ones. 

\item For CCs of clause-embedding predicates: Asking-whether diagnostic shows more differentiation than the other ones.
...
\begin{enumerate}
\item For NRRCs:  The stimuli with NRRCs convey two speech acts (asserted main clause/asserted NRRC vs. question/asserted NRRC). Since Exp 2 asks participants to judge what the speaker is asking about, the NRRCs are not candidates, resulting in the extreme not-at-issueness ratings. The other three diagnostics do not presume that one content is a question, but are in principle compatible with both asserted contents. 

If we go with this, this needs to be part of the discussion of Exps 1-4 already, and could be mentioned as a reason for why Exps 5-6 don't include NRRCs anymore.

No idea why Syrett/Koev found a difference in declarative diagnostic, leave to future research
...
\subsection*{Why might the different experiments/diagnostics give different results? \label{ssec:discussion-questions}}

This section develops some hypotheses for why the different diagnostics implemented in Exps.~1--4 give different results compared to one another and compared to the results of prior literature.

Starting with the contents of sentence-final and -medial NRRCs, recall that \citealt[Exp.~2]{syrett_experimental_2015} found that the content of sentence-final NRRCs was relatively more at-issue than that of sentence-medial ones, using a direct dissent diagnostic in which participants had to choose to directly dissent either with the content of the NRRC or of the main clause. This difference was not replicated in the results of our Exps.~1-4: the results of Exp.~4 (`yes, but') suggested the opposite result, and the result was not obtained even in Exp.~2 (`asking whether') which otherwise suggested great differentiation between the CCs of the five clause-embedding predicates investigated. 


Why might the results of Exp.~2 (`asking whether'), which otherwise showed relatively strong differentiation between the seven contents investigated,  not suggest a difference between the two types of NRRCs? We hypothesize that this is due to the stimuli of the diagnostic presenting the NRRCs as assertions, while the diagnostic asks participants to judge whether the speaker is asking about the content of the NRRC. Consider the sample stimuli in \ref{ex:nrrc-AW}, which convey two distinct speech acts: the content of the NRRC is an assertion and the main clause content is an (information-seeking) question (see, e.g., \citealt{potts_logic_2005,koev_apposition_2013,syrett_experimental_2015,krifka_embedding}). 


\ex.\label{ex:nrrc-AW} Sentence-medial NRRC: \emph{Is Greg, who bought a car, envied by his neighbor?}
\\ Sentence-final NRRC: \emph{need example from experiment}
\\ Question for participants: Is the speaker asking whether Greg bought a car?
\z.

Given that the NRRCs are asserted, it perhaps comes as no surprise that participants did not judge the speaker to be asking about the contents of the NRRCs, as suggested by the very low mean responses for both types of NRRC in Exp.~2. Possibly a floor effect.

In Exps.~1, 3 and 4, on the other hand, the stimuli with the NRRCs consisted of two assertions and participants had to rate whether uttered declarative was a natural response to the question about the NRRC (Exp.~1, QUD diagnostic), whether the NRRC could be directly dissented with (Exp.~3, direct dissent), or choose between direct and indirect dissents with the content of the NRRC (Exp.~4, `yes, but'). Of these three experiments, the diagnostics in Exps.~3 (direct dissent) and Exp.~4 (`yes, but') come closest to the diagnostic of Exp.~2 in \citealt{syrett_experimental_2015}. As noted above, the results of Exp.~3 did not replicate \citepos{syrett_experimental_2015} result, and those of Exp.~4 suggested the opposite result.  There are a number of differences between the three diagnostics that may have contributed to these different results: the stimuli, the response task, the response options. It is a pressing question for future research to identify whether the results obtained in the three experiments replicate and, if yes, why these three diagnostics give distinct results.

\jt{If we go with this, this could be discussed here or after Exps 1-4 already; if the latter, it could be mentioned as a reason for why Exps 5-6 don't include NRRCs anymore.}

\jt{In writing the next passage, I noticed that we do not replicate the results of \citealt{degen_projection_2025}, on the basis of which the five clause-embedding predicates were chosen. So I'm writing this bit now for the discussion part of Exps 1--4, where this should already be pointed out.}
...
One of the salient differences between the `asking whether' diagnostic (Exp.~2) and the other diagnostics (Exps.~1,3,4) is that the stimuli under the `asking whether' diagnostic are interrogatives whereas they are declaratives in the other three diagnostics. We hypothesize that this difference in sentence type may have contributed to the results. Recall that we selected the five clause-embedding predicates because their CCs differed greatly under the asking-whether diagnostic in \citealt{degen_projection_2025}. In contrast, the CCs of the five predicates exhibit a rather uniform property in declaratives. Specifically, \citealt[Exps.~2]{degen_are_2022} investigated the extent to which the CCs of the same 20 clause-embedding predicates (and same 20 items) as in \citealt{degen_projection_2025} are inferred from utterances of declaratives. The CCs of the five clause-embedding predicates investigated in our Exps.~1--4 all received very high mean inference ratings on a scale from 0-1 (with the lowest being .9 for \emph{confess}). What these results mean is that an interpreter who utters a declarative with one of these five clause-embedding predicates in one of our Exps.~1, 3, 4 is very likely to infer that the CC is true. 

We hypothesize that this inference to the truth of the CC may have contributed to the lesser differentiation between the CCs of the five clause-embedding predicates in Exps.~1, 3, 4. When a participant reads a speaker's utterance of a declarative with one of the five predicates in Exps.~1, 3, 4, they are not only likely to infer the truth of the CC but they are also likely to attribute to the speaker a commitment to the truth of the CC -- after all, if they speaker hadn't intended to convey such a commitment, they would not have used one of these five predicates. This, in turn, means that a participant in Exps.~1, 3, 4 is likely to take the speaker to be committed to both the content of the main clause and the CC. 

Let's consider how the hypothesis that participants take the speaker to be committed to both the content of the main clause and the CC plays out under the three diagnostics in Exps.~1, 3, 4. Exp.~1 (QUD diagnostic) asks participants to rate the naturalness of the declarative response to a question that is addressed by the CC. The results of Exp.~1 suggest that participants rate the dialogues with \emph{know, confess, discover} and \emph{confirm} to be rather natural (we leave \emph{be right} out here for  the reasons discussed above), which may be due to the participant taking the speaker to be committed to the CCs, which are therefore possible possible asserted contents. Exp.~3 (direct dissent) and Exp.~4 (`yes, but') ask participants to rate the naturalness of a direct dissent of the CC or to choose between direct and indirect dissents to the CC. If these diagnostics are sensitive to the availability of the CC for anaphoric reference, the hypothesis that the speaker is committed to each of these CCs may also mean that each of the CCs is available for anaphoric reference. \jt{still a bit wonky but running out of steam}

Another difference between the `asking whether' diagnostic (Exp.~2) and the other diagnostics (Exps.~1,3,4) is that the `asking whether' diagnostic asks participants to judge the speaker's intended speech act whereas the other three diagnostics ask participants to rate the naturalness of a dialogue (Exp.~1), the naturalness of a direct dissent (Exp.~3) or choose between direct and indirect dissent options (Exp.~4). One possibility is that the response task is what matters. This is why we ran Exps.~5 and 6.
