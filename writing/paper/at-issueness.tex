...
\bigskip \large
Word count (including references, excluding abstract and supplements): 11,389
}
...
One influential definition assumes that discourse is structured around addressing a \textit{Question Under Discussion} (QUD) (\citealt{roberts_information_2012,ginzburg_interrogatives_1996}) and treats at-issue content as the part of an utterance intended to address that question (\citealt{amaral_review_2007,simons_what_2010}):
...
\noindent Another definition, given in \Next, is grounded in understanding assertion as a proposal to update the common ground (\citealt{stalnaker_assertion_1978,clark_contributing_1989,ginzburg_resolving_1995,farkas_reacting_2010}). Here, the at-issue content of an assertion is proposed for addition to the common ground (\citealt{koev_apposition_2013}), while not-at-issue content is presupposed (already entailed in the common ground; \citealt{stalnaker_presuppositions_1973,stalnaker_common_2002}), or added without being proposed (\citealt{murray_evidentiality_2010,murray_varieties_2014,anderbois_crossing_2010,anderbois_at-issue_2015}).
...
Four commonly used diagnostics are illustrated in (\pref{qud}--\pref{yesbut}) for sentence-medial appositive non-restrictive relative clauses (NRRCs), which are typically taken to contribute not-at-issue content (\citealt{potts_logic_2005}).
...
These diagnostics can be understood as targeting properties (i)--(iii). The QUD-diagnostic \ref{qud} (\citealt{amaral_review_2007,lee_evidentiality_2011,tonhauser_diagnosing_2012,chen_presuppositions_2024}) targets property (i) by testing whether the target content can felicitously address an established QUD. It presents the content in a response to a question that makes the the target content relevant and elicits ratings of question-answer fit. Ratings are expected to be high when the content can be construed as at-issue and low otherwise. Accordingly, ratings are expected to be low for the appositive NRRC in \ref{qud}.

The `asking whether' diagnostic \ref{aw} (\citealt{tonhauser_how_2018,solstad_cataphoric_2024,degen_projection_2025}) targets property (ii) by probing whether the target content determines the alternatives of a polar question. It presents the target content in a polar question, and elicits judgments about whether the speaker is asking whether that content is true. These judgments are expected to be high for at-issue content and low otherwise. Accordingly, participants are expected to judge that the speaker is not asking about the NRRC content in \ref{aw}.

The direct-dissent diagnostic \ref{dd} (\citealt{faller_semantics_2002,faller_evidentiality_2006,papafragou_epistemic_2006,amaral_review_2007,murray_evidentiality_2010,murray_varieties_2014,anderbois_crossing_2010,anderbois_at-issue_2015,tonhauser_diagnosing_2012,syrett_experimental_2015}), and the `yes, but' diagnostic \ref{yesbut} (\citealt{xue_correlation_2011,cummins_backgrounding_2013,destruel_cross-linguistic_2015}) target property (iii). Both present the target content in a declarative sentence and assume that only at-issue content can be directly assented or dissented with, whereas dissenting with not-at-issue content requires more indirect discourse moves.
%
The direct-dissent diagnostic elicits naturalness ratings for a response directly dissenting with the target content, which are expected to be high for at-issue content and low otherwise. Accordingly, low ratings are expected for the NRRC content in \ref{dd}.
%
The `yes, but' diagnostic presents a forced choice between \emph{`no'}, \emph{`yes, but'}, and \emph{`yes, and'} when denying the target content. Under the assumption that the \emph{`yes'}-responses provide indirect denials suitable for dissenting with not-at-issue content (\citealt{xue_correlation_2011}), participants are expected to prefer one of the \emph{`yes'}-responses when denying not-at-issue content, such as the NRRC content in \ref{yesbut}.%
%
\footnote{%
Predictions for at-issue content are less clear and may be context-dependent. Although at-issue content permits direct dissent, it does not require it, and indirect dissent is also possible (see, e.g., \citealt{cummins_backgrounding_2013} on \citepos{shanon_two_1976} \enquote{Hey, wait a minute} diagnostic).
...
\citet{tonhauser_diagnosing_2012} suggests that diagnostics targeting properties (i)--(iii) can be taken to probe a common underlying notion of at-issueness, based on the QUD-based definition.
...
In contrast, views that take different diagnostics to probe distinct phenomena are motivated by empirical divergences between them. For example, although appositive NRRCs are widely taken to contribute not-at-issue content, a forced-choice continuation study by \citet{syrett_experimental_2015} found that sentence-final appositive NRRCs pattern as more at-issue than medial ones, under a version of the direct-dissent diagnostic, which targets property (iii). \citet{snider_anaphoric_2017} argues that diagnostics targeting property (iii) are sensitive to this medial-final distinction, but diagnostics targeting properties (i) or (ii) are not. Similar diagnostic differences have been reported for sentence-final slifting parentheticals (\citealt{koev_notions_2018}) and evidential inferences (\citealt{korotkova_evidential_2020}).

Such differences have been interpreted in two main ways. \citet{koev_notions_2018} takes them to indicate that at-issueness is not a uniform notion. On this view, the QUD-diagnostic (property i) targets a QUD-based notion of at-issueness, while the direct-dissent and `yes but' diagnostics (property iii) probe a proposal-based notion. The two definitions are thus taken to characterize distinct discourse phenomena, which may diverge empirically.
%
Another view (\citealt{snider_anaphoric_2017,snider_at-issueness_2017,snider_distinguishing_2018,korotkova_evidential_2020}) maintains a unified notion of at-issueness, arguing that diagnostics targeting properties (i) and (ii) genuinely probe at-issueness,
% QUD at-issueness,\footnote{Property (ii) can be seen as a forward-looking counterpart of property (i) (\citealt{snider_anaphoric_2017}): property (i) concerns addressing an established QUD, and property (ii) concerns determining the alternatives introduced by a question. Under standard assumptions that explicit questions introduce or update the QUD (\citealt{roberts_information_2012,ginzburg_interrogatives_1996}), property (ii) aligns with the QUD-based definition, but is incompatible with the proposal-based definition, which applies only to assertions.}
whereas property-(iii) are instead sensitive to the availability of the target content for propositional anaphora. Property-(iii) diagnostics are thus taken to reflect whether the target content can provide antecedents for response particles (\emph{yes/no}) or propositional pro-forms (e.g., \emph{that} in \emph{that's not true}), which are present in the direct-dissent and `yes but' diagnostics.
...
Initial experimental work on these questions has yielded mixed answers. As noted above, \citet{syrett_experimental_2015} found a positional effect for appositives under a version of the direct-dissent diagnostic. Participants saw declarative sentences introducing the target content and chose between two \emph{no}-responses expressing direct dissent, targeting the appositive or the main-clause content \Next.

\ex. \citealt{syrett_experimental_2015}, p.~17
\a.[A:] \emph{My friend Sophie, a classical violinist, performed a piece by Mozart.}
...
Beyond appositives, \citet{tonhauser_how_2018} compared the `asking whether' and `are you sure?' diagnostics across several English expressions, including sentence-medial appositives and complements of the clause-embedding predicates \emph{annoyed, discover} and \emph{know} \Next.
...
The `are you sure?' diagnostic presents participants with dialogues like \Next and collects ratings about whether the response addresses the question (c.f. the `really' test of \citealt{shanon_two_1976}).
...
Subsequent work shows that the `asking whether' diagnostic can nonetheless detect fine-grained differences. \citet{degen_projection_2025} found fine-grained at-issuness differences among the contents of the complements of 20 clause-embedding predicates in English. Fig.~\ref{fig:dtglossa} shows the mean `asking whether' ratings for the 20 predicates used in their study, ordered by mean rating. 
\begin{figure}[h]
  \centering
...
Each of the seven contents in \ref{stims} was instantiated as one of the seven items in \ref{items} in each of the four experiments.
...
Exps.~1--4 were designed to compare four at-issueness diagnostics that have been widely used in the literature. To assess whether the diagnostics yield consistent findings when applied to the same target contents, we asked whether they reproduce contrasts reported in prior work, particularly positional effects for appositive NRRCs, and fine-grained differences among the contents of the complements of clause-embedding predicates. The results of Exps.~1--4 suggest that the four diagnostics do differ in whether they reproduce these contrasts, and in the relative order of the seven contents investigated. Three main differences between the four experiments emerge:
...
\begin{enumerate}
  ...
  \item The diagnostics differ in the relative order of the seven contents---most notably in the case of the complement of \emph{be right}, which ranks low under the QUD diagnostic but high under the other diagnostics.
...
The diagnostics, as implemented here, yield different rank orderings of contents. The most pronounced ranking difference concerns the complement of \emph{be right}: it receives the lowest ratings in Exp.~1 (QUD diagnostic)  and high ratings in the other experiments.
...
To distinguish between these possibilities, Exps.~5--6., reported in the next section, compare the `asking whether' diagnostic to a diagnostic that also embeds the target content in a polar question but elicits naturalness ratings to assess at-issueness. Both experiments yielded similarly fine-grained distinctions among contents, supporting the conclusion that the increased differentiation observed in Exp.~2 (`asking whether') is driven primarily by interrogative embedding rather than by the response task.
...
\begin{enumerate}
  \item \textbf{Question-embedding hypothesis:} Differentiation between contents is greater when contents are embedded in a polar question than in a declarative sentence.
...
This diagnostic assumes that at-issue content determines the question alternatives (property (ii)). Participants read a dialogue in which one speaker asks a polar question containing the target content, and then rate the naturalness of a \emph{yes} response that asserts the target content. Such a response is felicitous only if the target content provides a possible answer, that is, a question alternative (\citealt{von_stechow_focusing_1991}). Accordingly, naturalness ratings are expected to be high when the target content is at-issue and low otherwise.

Under the question-embedding hypothesis, the greater differentiation in Exp.~2 (`asking whether') is attributed to presenting the target content in a polar question rather than a declarative. This predicts that Exps.~5 and 6, which also embed target content in questions, should both show high differentiation. Under the response-task hypothesis, the greater differentiation in Exp.~2 is attributed to probing speaker intentions rather than collecting metalinguistic judgments such as naturalness ratings. On this view, Exp.~5 (`asking whether') should show high differentiation, whereas Exp.~6 (direct response) should not, since it uses naturalness judgments, like Exps.~1, 3, and 4.
...
Critical stimuli in both experiments were constructed by combining each of the 20 clause-embedding predicates in \ref{ex:predicates} with one of 20 items (see Supplement \ref{supp:a-clauses}). Both experiments included the same six control stimuli, where target contents were expected to be at-issue (see Supplement \ref{supp:control56}).
...
In sum, the two diagnostics as implemented Exps.~5 and 6 (`asking whether' and direct-response) yielded highly similar results for the 20 contents investigated, including fine-grained by-predicate differentiation. Although they differed in response task, both experiments presented the target content in a question and both yielded strong by-content differentiation. This pattern supports the question-embedding hypothesis over the response-task hypothesis.
...
Nonetheless, small differences between the results of Exps.~5 and 6 remain, for example in which by-content differences reached significance and in the precise rank ordering of contents. Since the two experiments used identical embedding contexts and both tested property (ii), which has been linked to a QUD-based definition, these differences likely stem from the difference in response task, suggesting that response task differences can affect at-issueness judgments in subtle ways.
...
The experiments reported in this paper provide a clear answer to the first question: the diagnostics did not yield consistent results. Across Exps.~1--4, they differed in the extent to which they replicated previously reported differences among contents. A key methodological implication is therefore that at-issueness diagnostics cannot be treated as interchangeable.
...
The remainder of this section develops these points in more detail. \S~\ref{ssec:discussion-methods} discusses the methodological implications of diagnostic non-convergence. \S~\ref{ssec:discussion-questions} considers the role of question embedding and speaker commitment in producing greater by-content differentiation. \S~\ref{ssec:discussion-differences} then returns to the broader question of whether the observed diagnostic differences support distinct underlying notions of at-issueness.

\subsection{Methodological implication \label{ssec:discussion-methods}}
A methodological implication of the diagnostic differences we found is that diagnostics cannot be treated as interchangeable. Theoretical claims based on one diagnostic should be understood relativize to that diagnostic and results obtained with one task may not always generalize to others (see also \citealt{snider_anaphoric_2017,snider_at-issueness_2017,snider_distinguishing_2018,koev_notions_2018,korotkova_evidential_2020}). Therefore, empirical investigations should carefully consider that the outcomes of at-issueness diagnostics may depend on whether the target content was presented in a declarative sentence or polar question, contextual requirements of the expressions used, or the particular task design.

\subsection{Speech act, at-issueness, and speaker commitments \label{ssec:discussion-questions}}
The empirical generalization that diagnostics that present target contents in declarative sentences show reduced differentiation, whereas diagnostics using polar questions show greater differentiation raises the question of why the embedding context should have such a strong effect on diagnostic outcomes. We hypothesize that this pattern reflects a ceiling effect in assertion-based diagnostics, driven by how different speech acts relate the main-clause content to speaker commitment. We assume that the declaratives in these diagnostics are interpreted as assertions, which involve speaker commitment to the truth of the at-issue main-clause content; and the polar questions are interpreted as information-seeking questions, which do not involve speaker commitment. Because content that is less at-issue is more (likely to be) projective (\citealt{tonhauser_how_2018,degen_projection_2025}), not-at-issue content is also associated with speaker commitments. As a result, the relationship to speaker commitments of not-at-issue content differs more sharply from the at-issue main clause content in polar questions than in assertions.

To illustrate this key contrast, consider a polar question like \Next, where the at-issue content determines the set of alternatives under consideration \Next[a], but the speaker is not committed to any of these alternatives. In contrast, not-at-issue content, like the NRRC content in \Next[b], is more likely to project, that is, more likely to be treated as true throughout the context set, and therefore as part of the speaker's commitments \citep{abusch_presupposition_2010,simons_what_2010,abrusan_predicting_2011,tonhauser_how_2018,degen_projection_2025}. 
...
Because the various diagnostics tested in this paper are grounded in different assumptions about what it means for content to be at-issue, the observed differences between diagnostics also bear on the broader question of whether existing diagnostics track a common underlying notion of at-issueness or operationalize distinct phenomena. This subsection considers how the differences between diagnostics we observed relate to patterns that would be expected based on views on this question: views that assume that different diagnostics target a unified phenomenon of at-issueness (e.g., \citealt{tonhauser_diagnosing_2012,anderbois_at-issue_2015,faller_discourse_2019}), different underlying notions (\citealt{koev_notions_2018}), or that some diagnostics target at-issueness, while others diagnose the availability of the target content for propositional anaphora (\citealt{snider_at-issueness_2017,snider_at-issueness_2017,snider_distinguishing_2018,korotkova_evidential_2020}). These positions differ with respect to how different diagnostics relate to the two definitions of QUD at-issueness and proposal at-issueness, repeated here:
...
The literature differs in how the theoretical assumptions behind these two definitions should be interpreted. And the different positions lead to different expectations about what kind of empirical divergence would count as evidence to support them. One position is that the two definitions characterize different perspectives on a unified underlying phenomenon (e.g., \citealt{tonhauser_diagnosing_2012,anderbois_at-issue_2015,faller_discourse_2019}).  This unified view would be supported by convergent results across diagnostics, but it is also consistent with diagnostic differences, if they can be attributed to factors that are orthogonal to differences in at-issueness itself. 

Another position is that the two definitions characterize two different underlying phenomena (\citealt{koev_notions_2018}). This dual-notion view of at-issueness predicts that at least some diagnostic differences reflect a split between the diagnostics assumed to align with the proposal-based definition (i.e., the dissent-based diagnostics from Exps.~3--4) and those more in line with the QUD-based definition (i.e, the QUD diagnostic from Exp.~1; the `asking whether' diagnostic from Exps.~2 and 5; and the direct-response diagnostic from Exp.~6).

A third position is that some diagnostic differences instead arise because the diagnostics including polarity particles (e.g., \emph{yes/no}) or propositional pro-forms (e.g. \emph{that} in \emph{that's not true}) diagnose the anaphoric availability of the target content for these propositional anaphoric expressions, and only diagnostics without propositional anaphora genuinely diagnose at-issueness (\citealt{snider_anaphoric_2017,snider_at-issueness_2017,snider_distinguishing_2018,korotkova_evidential_2020}). This anaphoric-accessibility view predicts systematic differences between diagnostics that involve propositional anaphora, like the dissent-based diagnostics (Exps.~3--4) and the direct-response diagnostic (Exp.~6), and those that do not, like the QUD diagnostic (Exp.~1) and the `asking whether' diagnostic (Exps.~2 and 5).

In the following, we discuss whether our results provide support for any of these views. 

First, the clearest diagnostic difference we found---whether target contents are presented in polar questions or declaratives---may appear related to the distinction between QUD-based and proposal-based definitions of at-issueness, since both involve a contrast between questions and assertions. However, as discussed in \S \ref{ssub:differentiation}, the QUD-diagnostic (Exp.~1) distinguishes between these factors: it assumes the QUD-based definition, but presents the target content in a declarative sentence. Contrary to what would be expected based on the dual-notion view, the QUD-diagnostic patterned with the dissent-based diagnostics (Exps.~3 and 4) in showing low by-content differentiation. The observed pattern is also not expected based on the anaphoric-availability view. the direct-response diagnostic from Exp.~6 uses the anaphoric polarity particle \emph{yes} to diagnose at-issueness, but it shows high by-content differentiation, and therefore does not pattern with the dissent-based diagnostics in Exps.~3 and 4, which also involve propositional anaphora. Therefore, the main contrast we found cuts across groupings expected based on views that assume diagnostics target distinct underlying phenomena. In \S\ref{ssec:discussion-questions}, we argued that this contrast can be plausibly explained independently of distinctions in at-issueness, which would be consistent with a unified-notion view. However, while it is not directly predicted by either the dual-notion or anaphoric-availability views, the pattern remains in principle compatible with all three positions.

Second, the diagnostics differ in how they interact with contextual requirements imposed by particular expressions. A clear example is the behavior of \emph{be right} under the QUD diagnostic, where low QUD-match ratings do not plausibly reflect not-at-issueness, but instead arise from a pragmatic incompatibility: \emph{be right} presupposes a discourse structure that conflicts with the assumptions made by the diagnostic (see \S~\ref{ssub:be-right}). A related point is raised by \citet{snider_at-issueness_2017,snider_distinguishing_2018} who emphasized that the direct-dissent and `yes but' diagnostic involve propositional anaphora in the interpretation in the response particles \emph{yes/no}. Different diagnostics impose different requirements on discourse structure, common ground, and anaphoric accessibility, all of which can independently affect acceptability judgments. This supports the conclusion that some differences between diagnostics arise from interactions with expression-specific contextual requirements rather than from differences in at-issueness per se.

Third, some small differences between diagnostics can be attributed to response-task effects. This is evident in the subtle differences between Exps.~5 and 6 (see \S~\ref{ssec:5-6-results}), which differed only in response format, and in our failure to replicate \citeposs{syrett_experimental_2015} \citeyear{syrett_experimental_2015} reported positional effect for appositive NRRCs (see \S~\ref{ssub:appositives}). While they found sentence-final appositives to be more at-issue than sentence-medial ones under a version of the direct-dissent diagnostic, our assent/dissent-based \emph{yes, but} diagnostic in Exp.~4 showed a difference in the opposite direction, while the other experiments found no difference between them (Exp.1--3). Taken together, these results provide no evidence for a stable notion of at-issueness under which sentence-final appositives are systematically more at-issue than medial ones (or the other way round), and they highlight how even small task differences can affect outcomes.
...
This paper presented a systematic experimental comparison of at-issuness diagnostics, motivated by two questions about experimental diagnostics of at-issueness: whether different diagnostics yield consistent results when applied to the same contents, and whether any differences between diagnostics support the view that they are sensitive to distinct underlying phenomena.

Across six experiments, we tested five diagnostics: the QUD diagnostic, the `asking whether' diagnostic, the direct-dissent diagnostic, the `yes, but' diagnostic, and the direct-response diagnostic. Our results provide experimental confirmation for claims from the theoretical literature that there are empirical differences between at-issueness diagnostics (\citealt{snider_anaphoric_2017,snider_at-issueness_2017,snider_distinguishing_2018,koev_notions_2018,faller_discourse_2019,korotkova_evidential_2020}). Since the diagnostics did not yield fully consistent results, they cannot be treated as interchangeable.

The experiments also revealed a robust source of variation: diagnostics that presented target contents in polar questions showed greater by-content differentiation than diagnostics that presented the same contents in assertions. We argued that this pattern shows that illocutionary force and speaker commitments affect the outcomes of at-issueness diagnostics, and that these factors should be taken into account when comparing diagnostic results.

Our findings do not provide evidence that different diagnostics target distinct underlying phenomena. Although the diagnostics differed empirically, the clearest differences did not align in any straightforward way with existing proposals distinguishing QUD-based at-issueness, proposal-based at-issueness, and anaphoric availability. 
Because many of the observed differences can be attributed to factors other than the theoretical distinctions currently discussed in the literature, our results are more compatible with the view that the diagnostics offer different, partially overlapping windows onto a single discourse phenomenon that interacts with multiple discourse factors in complex and sometimes subtle ways (e.g., speaker commitments, illocutionary force, contextual felicity requirements, and the given alternatives in forced-choice tasks).
