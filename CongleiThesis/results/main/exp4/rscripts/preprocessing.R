...
d = d %>%
  select(participant_id,response,trial_type,trial_index)
nrow(d) #800

# unpack demographics info
dg <- d %>%
  filter(trial_type == "survey") %>%
  select(c(participant_id,response))
#view(dg)
...
d = left_join(d, dg, by = "participant_id")
view(d)
...
trialCount = d %>% 
  select(trial_index, participant_id) %>% 
  unique() %>% 
...
d <- droplevels(subset(d, !(d$participant_id %in% outlier_Turkers1$participant_id)))
length(unique(d$participant_id)) #4 excluded

d <- droplevels(subset(d, !(d$participant_id %in% outlier_Turkers2$participant_id)))
