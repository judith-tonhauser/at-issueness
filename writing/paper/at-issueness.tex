...
\subsection{Outlook and how we address the research questions}
...
As shown in Fig.~\ref{fig:results}, Exp.~2 (`asking whether') largely reproduces the pattern from \citet{degen_projection_2025} (LIST), except that \emph{confess} and \emph{discover} do not differ.
\jt{I would like to suggest that we use language here to express the previous finding (this should be in Section 1 already), not $<$.} 
...
One factor that might explain the difference between our findings and those of \citet{syrett_experimental_2015}, apart from the stimuli, is the response task. Although both studies tested appositive NRRCs using dissent-based diagnostics presenting the target content in an assertion, their\jt{unclear antecedent} forced-choice implementation differs from our Exps.~3 and 4. In Exp.~3 (direct dissent), we elicited naturalness ratings rather than forced-choice judgments. Even\jt{why ``even''? I don't get the scale} in Exp.~4 (`yes, but'), which also used a forced-choice continuation task, the response options were different. In our implementation (based on \citealt{xue_correlation_2011}), all options rejected the target content, and participants chose between expressing direct dissent (\emph{no}), indirect dissent (\emph{yes, but}), or assent without contrast (\emph{yes, and}). In Syrett \& Koev's study, all options expressed direct dissent (\emph{no}), and participants chose whether to reject the appositive content or the main clause content, as illustrated in \Next.\jt{this should be in section 1}
...
We hypothesize that this is because \emph{be right} in \Last presupposes\jt{I am not comfortable with this formulation as I do not know what it means to presuppose.} that Danny has previously committed to the proposition that \enquote{Lucy broke the plate} \jt{Noa has also looked into the meaning of ``be right''. Please also talk to her about relevant references and point out these two to her -- though, where does Abusch 2010 talk about be right?}\lh{on p. 50, when discussing "near-minimal pairs"}(\citealt{abusch_presupposition_2010,anand_factivity_2014}). In the three diagnostics in \Last[b--d], no previous discourse context is given, so this presupposition can be accommodated. In contrast, the preceding question in the QUD-diagnostic \Last[a] conflicts with the presupposition, making it difficult to accommodate.\jt{I do not follow: Why couldn't the dialogue have been: Danny: Lucy broke the plate. Nora: What did Lucy break? Leo:...?} Specifically, we hypothesize that \emph{be right} signals that the question \enquote{whether Lucy broke the plate} is salient in the preceding discourse.\jt{Is this a second felicity requirement? you call it a presupposed question below. I am, again, not comfortable with that.} This allows us to understand the ill-formedness based on QUD-based discourse structure, because this presupposed question is a subquestion (in the sense of \citealt{roberts_information_2012}) of the question in \Last[a] \enquote{What did Lucy break}, since every complete answer to the latter entails an answer to the former. The progression from the presupposed subquestion question to the explicitly given superquestion violates Roberts' constraint on QUD stacks (p. 6:15), which allows only the reverse order (from superquestions to subquestions).
...
To explain the greater by-content differentiation of the `asking whether' diagnostic, we therefore focus on properties that set it apart from the other three diagnostics. Two differences are particularly salient: First, it is the only diagnostic where the target content itself occurs in a polar question, whereas in the other diagnostics, the target content appears in a declarative assertion. Second, it is the only diagnostic where participants are asked directly about the intentions of the speaker, while the other ones collect measures that are more indirectly tied to semantic interpretation, such as acceptability ratings and continuation choices.\jt{Are acceptability ratings ratings about semantic interpretation? Why ``semantic''? Perhaps a better contrast here is between speaker intentions and participants' meta-linguistic judgments?} Either of these properties might be important for explaining the greater differentiation.

These considerations motivate two hypotheses,\jt{I think it's weird to mention two hypotheses but not present them, especially given how much other detail is given about what's next}  which we tested in Exps.~5--6. The following section reports on these experiments, which compare the `asking whether' diagnostic to a diagnostic that also embeds the target content in a polar question but elicits naturalness ratings for a direct response to assess at-issueness. Both experiments yielded similarly fine-grained distinctions among contents, supporting the conclusion that the increased differentiation observed in Exp.~2 is driven primarily by interrogative embedding rather than by the response task.

\section{Experiments 5 and 6 \label{sec:3_more-experiments}}
%
Exps.~5 and 6 investigate\jt{were designed to investigate} whether the greater differentiation observed in Exp.~2 (`asking whether') was due to (i) embedding target contents in polar questions or (ii) the response task, testing two hypotheses:
\begin{enumerate}
  \item \textbf{Question-embedding hypothesis:} Differentiation between contents is greater when contents are embedded in a polar question than in a declarative assertion.\jt{`polar question' is a sentence type; `declarative assertion' is both sentence type and speech act}
...
Both experiments measured at-issueness of the contents of the complements of the 20 clause-embedding predicates from \citealt{degen_projection_2025}:\jt{perhaps also tell readers that this is the set shown in Fig 1?}
...
\lh{I don't think so, and if it should be the same in both, I would suggest omitting this latter criterion here, since it does not seem to make much of a difference.}\jt{fine by me}
...
Crucially, both experiments show fine-grained by-content differentiation. For instance, for the subset of predicates also tested in Exps.~1--4, Exps.~5 and 6 reproduce the ordering expected based on \citet{degen_projection_2025} and show pairwise differences in the predicted directions for adjacent contents (\emph{know} < \emph{discover} < \emph{confess} < \emph{confirm} < \emph{be right}).\jt{See my comment above} Overall, the results in Exps.~5 and 6 are similar in revealing fine-grained differences among many rank-adjacent predicates.
