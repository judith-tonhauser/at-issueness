...
We can understand the ill-formedness of \Last within a QUD-based framework of discourse structure (in the sense of \citealt{ginzburg_interrogatives_1996,roberts_information_2012,buring_d-trees_2003}). Within this framework, the contextual requirement of \emph{be right}, that a prior commitment be salient in the discourse, will result in a requirement that QUD addressed by the act of making this commitment addresses must be part of the QUD stack. In the case of \Last, this means that the question \enquote{Whether Lucy broke the plate} must be part of the QUD stack at the point where \emph{be right} is uttered.

This question is a subquestion of the question in \Last[b] \enquote{What did Lucy break}, since every complete answer to the latter entails an answer to the former. The progression from the presupposed subquestion question to the explicitly given superquestion violates \citepos{roberts_information_2012} constraint on QUD stacks (p. 6:15), which allows only the reverse order (from superquestions to subquestions).\footnote{This constraint on QUD stacks can be understood as an informativeness-constraint, which requires that discourse moves contribute towards increasing the information quantity in the context set. Accordingly, the exchange in \Last is acceptable only under an echo-question interpretation of Nora's question.}
